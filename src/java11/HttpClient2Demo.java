...
/**
 * 
 * Composition of http client allows using builder pattern to compose the instance
 *
 */
public class HttpClient2Demo {
  ...
  private static void synchronousRequest()
    ...
    HttpRequest request = HttpRequest.newBuilder()
        .uri(new URI("https://postman-echo.com/get?foo1=bar1&foo2=bar2")).build();
  ...
  private static void asynchronousRequest()
    ...
    List<URI> uris = Stream
        .of("https://postman-echo.com/get?foo1=bar1&foo2=bar2",
    ...
    CompletableFuture<Void>[] futures =
        uris.stream().map(uri -> buildAsyncRequest(client, uri)).toArray(CompletableFuture[]::new);
  ...
  private static CompletableFuture<Void> buildAsyncRequest(HttpClient client, URI uri) {
    ...
    return client.sendAsync(request, HttpResponse.BodyHandlers.ofString()).thenApply((response) -> {
      ...
      System.out.println(response.body());
      return response;
    }).thenApply(HttpResponse::statusCode).thenApply(statusCode -> statusCode == 200)
        .exceptionally(ex -> false).thenAccept((success) -> {
          if (success) {
