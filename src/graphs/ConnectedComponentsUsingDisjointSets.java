...
/**
 * Given a graph, find the total number of connected components in it using disjoint sets algorithm
 *
 */
public class ConnectedComponentsUsingDisjointSets {
  ...
  public static void main(String[] args) {
    ...
    /**
     * initially all the nodes are made their own parent thereby initializing the whole parent array
     * with -1
     * 
     */
    ...
    for (GraphEdge edge : graph.getAllEdges()) {
      ...
      if (parent1 != parent2) {
        ...
        /**
         * check which ever node is parent to more nodes in graph, make it the parent of the other
         * 
         */
    ...
    /**
     * check total number of distinct parents after taking disjoint union set operation
     * 
     */
    Arrays.stream(parents).filter(parent -> (parent < 0)).forEach((parent) -> {
      list.add(parent);
    });
  ...
  /**
   * 
   * fetch parent of provided node, if the node itself is not it's own parent then recursively fetch
   * it. Negative value in parenta array at node's index denotes that node is it's own parent
   * 
   */
