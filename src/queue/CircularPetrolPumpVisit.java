 ...
 * 
 * The amount of petrol that every petrol pump has. Distance from that petrol pump to the next
 ...
 * 
 * The first point from where the truck can make a circular tour is 2nd petrol pump. Output should
 * be “start = 1” (index of 2nd petrol pump).
 *
 */
public class CircularPetrolPumpVisit {
  ...
  public static int findStartingPetrolPump(List<PetrolPump> list) {
    ...
    while (i < Integer.MAX_VALUE) {
      ...
      // add the current petrol pump in the queue and update remaining petrol accordingly
      queue.offer(pump);
      ...
      if (remainingPetrol < 0) {
        ...
        /**
         * start removing the visited petrol pumps from queue and set until we have sufficient
         * petrol to visit next petrol pump from current position
         * 
         **/
        ...
        /**
         * if queue is completely empty then start with the next petrol pump in the list and make
         * remaining petrol as 0
         * 
         **/
        if (queue.isEmpty()) {
          remainingPetrol = 0;
