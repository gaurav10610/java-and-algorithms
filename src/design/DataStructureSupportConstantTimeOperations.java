...
/**
 * Design a data structure which supports following operations with expected time complexity
 * 
...
class CustomDataStructure<T> {
  ...
  public boolean remove(T element) {
    ...
    if (!map.containsKey(element)) {
      ...
      /**
       * 
       * check if the element which is going to be deleted is stored at last position in list or
       * somewhere in between
       * 
       * a. if element is not the last element in list then replace it with the element stored at
       * last index in array and subsequently update the index of the last element in the map as
       * well then remove the last element from list and decrease the size
       * 
       * b. if element is the last element in the list then just remove it from the list and
       * decrease the size
       * 
       */
      if (listIndex != (size - 1)) {
